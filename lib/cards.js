 ...
 * for each card on the player hand
 */
function check(player, game) {
  ...
  _.merge(options,
    ...
    opts.fly(player),
    opts.placeHQ(player),
    opts.trade(player, game),
    mergeOptions
  );
  return options;
}

function mergeOptions(firstOpts, secondOpts) {
  firstOpts = firstOpts || [];
  return firstOpts.concat(secondOpts);
}
